...
| networkPolicy.pangolin.externalIngress.aiGateway | bool | `true` | Allow ingress on the Pangolin AI gateway port. On by default, like the external (3000) and dashboard (3002) ports: Traefik reaches the gateway by connecting to this port on the Pangolin Pod, so closing it makes every AI gateway route fail even though the port is published on the Service. |
