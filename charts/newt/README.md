...
![Version: 1.6.0](https://img.shields.io/badge/Version-1.6.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.16.0](https://img.shields.io/badge/AppVersion-1.16.0-informational?style=flat-square)
...
| global.metrics.adminAddr | string | `":2112"` | Address Newt binds for the metrics/admin server (NEWT_ADMIN_ADDR). AUTHORITATIVE for the metrics port: the container port, the Service targetPort and the prometheus.io/port annotation are all derived from it, so they cannot drift apart the way they did before (#16). ":2112" listens on all interfaces, which in-cluster Prometheus scraping requires. "127.0.0.1:2112" restricts to loopback and therefore disables scraping. |
...
| global.metrics.port | int | `9090` | DEPRECATED: use `adminAddr`, which is what Newt actually binds. Kept for compatibility: when set to a non-default value while `adminAddr` is still ":2112", it is used as the listen port so existing values files render the same port numbers as before. It no longer defines the container port on its own. |
...
| global.nativeMode.enabled | bool | `false` | Permission gate for native WireGuard interfaces. This grants nothing on its own: an instance becomes root and privileged only when it also sets useNativeInterface or useNativeMainInterface. Those settings are rejected while this is false. |
...
| global.resources | object | `{}` | Default resource requests/limits for every Newt container. Empty by default (#23): the chart imposes no sizing, so the container inherits any LimitRange or namespace default instead. A non-empty `newtInstances[].resources` always overrides this. CPU limits in particular are a deliberate choice, not a sensible default, because throttling a WireGuard data path degrades it silently. Example:   resources:     requests:       cpu: 100m       memory: 128Mi       ephemeral-storage: 128Mi     limits:       memory: 256Mi |
| global.revisionHistoryLimit | int | `3` | Number of old ReplicaSets/ControllerRevisions to retain. |
| global.securityContext | object | `{}` | Container-level securityContext override SECURITY NOTE: By default (when nativeMode is disabled), Newt runs as non-root:   runAsUser: 65534   runAsNonRoot: true   allowPrivilegeEscalation: false   readOnlyRootFilesystem: true   capabilities.drop: ["ALL"]  When global.nativeMode.enabled=true AND the instance requests a native interface:   runAsUser: 0 (root)   allowPrivilegeEscalation: true   privileged: true   capabilities.add: [NET_ADMIN, SYS_MODULE]  IMPORTANT: Do NOT set capabilities.drop when running as root - this is invalid. |
...
| newtInstances | list | `[{"acceptClients":false,"affinity":{"nodeAffinity":{},"podAffinity":{},"podAntiAffinity":{}},"allowGlobalOverride":false,"auth":{"createSecret":true,"envVarsDirect":false,"existingSecretName":"","keys":{"endpointKey":"PANGOLIN_ENDPOINT","idKey":"NEWT_ID","secretKey":"NEWT_SECRET"}},"authDaemon":{"caCertPath":"","generateRandomPassword":false,"keySecretKey":"","keySecretName":"","principalsFile":""},"blueprintData":"","blueprintFile":"","configFile":"","configPersistence":{"enabled":false,"existingClaim":"","fileName":"config.json","mountPath":"/var/lib/newt","type":"emptyDir"},"disableClients":false,"disableSSH":false,"dns":"","dockerSocket":{"enabled":false,"enforceNetworkValidation":false,"path":"/var/run/docker.sock"},"enabled":true,"enforceHcCert":false,"extraContainers":[],"extraEnv":{},"extraVolumeMounts":[],"extraVolumes":[],"generateAndSaveKeyTo":"","healthFile":"/tmp/healthy","hostNetwork":false,"hostPID":false,"id":"","initContainers":[],"interface":"newt","interfaceMain":"newt","keepInterface":false,"lifecycle":{},"logLevel":"INFO","metrics":{"annotations":{},"enabled":false,"path":"/metrics","podMonitor":{"annotations":{},"apiVersion":"monitoring.coreos.com/v1","enabled":false,"honorLabels":true,"interval":"30s","labels":{},"metricRelabelings":[],"namespace":"","path":"/metrics","portName":"metrics","relabelings":[],"scheme":"http","scrapeTimeout":"10s"},"port":9090,"portName":"metrics","prometheusRule":{"apiVersion":"monitoring.coreos.com/v1","enabled":false,"labels":{},"namespace":"","rules":[]},"service":{"annotations":{},"enabled":false,"port":9090,"portName":"metrics","type":"ClusterIP"},"serviceMonitor":{"apiVersion":"monitoring.coreos.com/v1","enabled":false,"interval":"30s","jobLabel":"","labels":{},"metricRelabelings":[],"namespace":"","relabelings":[],"sampleLimit":0,"scheme":"http","scrapeTimeout":"10s","targetLabels":[]},"targetPortName":""},"mtls":{"certPath":"/certs/client.p12","enabled":false,"mode":"pkcs12","p12Base64":"","pem":{"caPath":"/certs/ca.crt","clientCertPath":"/certs/client.crt","clientKeyPath":"/certs/client.key","secretName":""},"secretName":""},"mtu":1280,"name":"main-tunnel","namespace":{"create":null,"labels":{},"name":"","podSecurity":{"audit":"","enforce":"","warn":""}},"networkPolicy":{"components":{"custom":{"egress":[],"enabled":false,"ingress":[],"policyTypes":["Ingress","Egress"]},"defaultApp":{"egress":[],"enabled":false,"ingress":[],"policyTypes":[]},"dns":{"egress":[],"enabled":false},"kubeApi":{"egress":[],"enabled":false}},"enabled":null,"includeRuleSets":[],"mode":"merge","useGlobalComponents":{"custom":true,"defaultApp":true,"dns":false,"kubeApi":false}},"newtName":"","noCloud":false,"nodeSelector":{},"pangolinEndpoint":"","pingInterval":"","pingTimeout":"","podDisruptionBudget":{},"podSecurityContext":{},"port":"","preferEndpoint":"","provisioningBlueprintData":"","provisioningBlueprintFile":"","provisioningKey":"","replicas":1,"resources":{},"secret":"","securityContext":{},"service":{"annotations":{},"enabled":true,"enabledWhenAcceptClients":true,"externalTrafficPolicy":"","labels":{},"loadBalancerClass":"","loadBalancerIP":"","loadBalancerSourceRanges":[],"nodePorts":{"tester":"","wg":""},"port":51820,"testerPort":51821,"type":"ClusterIP"},"serviceAccount":{"annotations":{},"automountServiceAccountToken":false,"create":null,"name":""},"tolerations":[],"topologySpreadConstraints":[],"udpProxyIdleTimeout":"","updown":{"enabled":false,"fileName":"updown.sh","mountPath":"/opt/newt/updown","script":""},"useCommandArgs":false,"useNativeInterface":false,"useNativeMainInterface":false}]` | List of Newt instances to deploy. Each instance can optionally override |
| newtInstances[0].acceptClients | bool | `false` | Gate the client Service and the automatic client ingress NetworkPolicy rule. This is a chart-side switch only: it no longer sets any Newt environment variable, because ACCEPT_CLIENTS was removed upstream in Newt 1.7.0 and replaced by DISABLE_CLIENTS. To actually refuse client connections set `disableClients: true`. |
...
| newtInstances[0].authDaemon.caCertPath | string | `""` | Path to the SSH CA certificate (AD_CA_CERT_PATH). |
| newtInstances[0].authDaemon.generateRandomPassword | bool | `false` | Generate a random password on start (AD_GENERATE_RANDOM_PASSWORD). |
| newtInstances[0].authDaemon.keySecretKey | string | `""` | Key inside `authDaemon.keySecretName` holding the pre-shared key. |
| newtInstances[0].authDaemon.keySecretName | string | `""` | Name of an existing Secret holding the auth daemon pre-shared key (AD_KEY). The key is only ever referenced, never rendered into the manifest. |
| newtInstances[0].authDaemon.principalsFile | string | `""` | Path to the SSH principals file (AD_PRINCIPALS_FILE). |
...
| newtInstances[0].disableSSH | bool | `false` | Disable Newt's built-in SSH server (DISABLE_SSH). Newt 1.13 replaced the separate auth daemon with an internal SSH server that is on by default. |
...
| newtInstances[0].interfaceMain | string | `"newt"` | Name of the native main WireGuard interface (INTERFACE_MAIN). Requires Newt >= 1.14.0. |
...
| newtInstances[0].namespace.create | string | `nil` | Whether to create the namespace for this instance. null means inherit namespace.create. |
...
| newtInstances[0].preferEndpoint | string | `""` | Prefer this endpoint when several are advertised (--prefer-endpoint). Newt reads this only from the CLI, so it requires useCommandArgs: true on this instance; the chart rejects the combination rather than letting it no-op. |
...
| newtInstances[0].resources | object | `{}` | Per-instance resource requests/limits. Overrides `global.resources` whenever it is non-empty, independently of `allowGlobalOverride`. Empty by default so the chart imposes no sizing; see `global.resources` for an example. |
...
| newtInstances[0].securityContext | object | `{}` | Container-level securityContext override SECURITY NOTE: By default Newt runs as non-root. Newt runs as root with privileges only when global.nativeMode.enabled=true AND the instance sets useNativeInterface or useNativeMainInterface. IMPORTANT: Do NOT set capabilities.drop when running as root - this is invalid. |
...
| newtInstances[0].udpProxyIdleTimeout | string | `""` | Idle timeout for proxied UDP flows, e.g. "90s" (NEWT_UDP_PROXY_IDLE_TIMEOUT). Empty uses the Newt default. |
...
| newtInstances[0].useNativeInterface | bool | `false` | Use the native WireGuard kernel interface (USE_NATIVE_INTERFACE). Requires global.nativeMode.enabled=true; the chart then runs this instance as root with NET_ADMIN and SYS_MODULE. |
| newtInstances[0].useNativeMainInterface | bool | `false` | Create the main WireGuard interface natively on the host instead of in netstack (USE_NATIVE_MAIN_INTERFACE). Requires Newt >= 1.14.0 and global.nativeMode.enabled=true; the chart then grants this instance the same privileges as useNativeInterface. |
