...
| newtInstances[0].preferEndpoint | string | `""` | Prefer this endpoint when several are advertised (PREFER_ENDPOINT). |
...
| newtInstances[0].useNativeMainInterface | bool | `false` | Create the main WireGuard interface natively on the host instead of in netstack (USE_NATIVE_MAIN_INTERFACE). Requires Newt >= 1.14.0 and a privileged container. |
